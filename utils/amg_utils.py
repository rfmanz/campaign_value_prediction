...
semtools = "SEM_TOOLS"
# schemas 
bd_s2cinternal = "BD_S2CINTERNAL"
cm_fb_campaign_management = "CM_FB_CAMPAIGN_MANAGEMENT"
#tbls
v_cm_fb_tonic_daily_combine = "V_CM_FB_TONIC_DAILY_COMBINE"


def peek(df, rows=None):
    if rows is None:
        concat1 = pd.concat([df.dtypes, df.T], axis=1).reset_index()
    else:
        concat1 = pd.concat([df.dtypes, df.iloc[:rows, :].T], axis=1).reset_index()
...
sf_config = {
    ...
    # 'warehouse': 'SEM_TOOLS',
    # 'database': 'SEM_TOOLS',
    # 'schema': 'CM_FB_CAMPAIGN_MANAGEMENT',
    'role': 'SEM_TOOLS',
    'user': os.getenv('SNOWFLAKE_ID'),
...
def views(
    database: str = 'SEM_TOOLS',
    schema: str = None,   
    views_or_tables: str = 'VIEWS'          
    ) -> List[str]:
    """
    example: views('SEM_TOOLS','CM_FB_CAMPAIGN_MANAGEMENT', 'tables')
    """
    df = eq(f"show {views_or_tables} in {database}.{schema}")         
    if df.empty and views_or_tables=="VIEWS":
        df = eq(f"show tables in {database}.{schema}") 
    return [*df['name']]
...
def check_table(
    ...
    """   
    check_table('SEM_TOOLS', 'CM_FB_CAMPAIGN_MANAGEMENT', 'CM_FB_CAMPAIGN_MANAGEMENT', ['campaign_id', 'campaign_name', 'campaign_status'])
    or 

    check_table(name='SEM_TOOLS.CM_FB_CAMPAIGN_MANAGEMENT.TONIC_CAMPAIGN')
    """
    database, schema, table = name.split('.') if name else (database, schema, table)
    cols = ",".join(cols).replace(",", ",\n") if cols and isinstance(cols, list) else '*'
    return peek(eq(f"select {cols} from {database}.{schema}.{table} limit 5;"),5)

def to_query(check_table_string : str) -> str:
    # Extract the parameters from the input string
    match = re.search(r'check_table\((.*), (.*), (.*)\)', check_table_string)
    if match:
        database, schema, table = match.groups()
        # Evaluate the parameters if they're not strings
        if not database.startswith('"') and not database.startswith("'"):
            database = eval(database).strip('"').strip("'")
        else:
            database = database.strip('"').strip("'")
        if not schema.startswith('"') and not schema.startswith("'"):
            schema = eval(schema).strip('"').strip("'")
        else:
            schema = schema.strip('"').strip("'")
        if not table.startswith('"') and not table.startswith("'"):
            table = eval(table).strip('"').strip("'")
        else:
            table = table.strip('"').strip("'")
        # Return the formatted query string
        query = 'peek(eq("select * from {}.{}.{};"))'.format(database, schema, table)
        # Remove single quotes from the query
